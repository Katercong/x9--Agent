...
## 2026-05-14 — v3.12.0（AI Agent + Desktop AI 助手优化）

> 纯内部优化，无 API 破坏性变更。廖的爬虫调用不受影响。

### Core AI Agent（`app/agent.py` + `app/llm.py`）

- `+` **文档内存缓存**：`load_docs()` 改为 mtime-sum 缓存，docs/ 文件不变时跳过磁盘 I/O；文件改动自动 invalidate
- `+` **Anthropic Prompt Caching**：provider type == anthropic 时，~43KB 系统提示自动包上 `cache_control: ephemeral`（5 分钟 TTL），重复提问 input token 成本降低约 90%；需在请求头加 `anthropic-beta: prompt-caching-2024-07-31`（已自动加）
- `~` **默认 temperature 0.3 → 0.1**：Agent 回答更确定性，减少编造；可在 `llm_feature` 表 `temperature` 字段覆盖
- `~` `_call()` / `_call_anthropic()` 的 `system` 参数现接受 `str | list`：list 格式用于 cache_control blocks（Anthropic 专用）；openai_compat provider 收到 list 时自动 flatten 为纯文本，向后兼容
- `~` Agent 返回字段新增 `cache_creation_tokens` / `cache_read_tokens`（Anthropic 缓存命中时有值）
- `-` 前端 `index.html` 硬编码的 `max_tokens: 1500` 已删除，现以 `llm_feature.max_tokens` 配置为准

### Desktop AI 助手（`desktop/backend/routers/shared.py` + `desktop/backend/ui/app.js`）

- `+` **System Prompt 扩展**：从 3 句泛泛描述扩展为结构化知识（7 个页面功能 + 4 个关键概念 + 行为约束），助手现在能正确回答页面导航、Fit Level / Queue 含义、AI 润色操作等问题
- `+` **Markdown 渲染**：`renderMarkdown()` 函数上线，AI 回复支持 `**加粗**`、`` `代码` ``、`-` 列表，用户输入仍走 `escapeHtml()` 防 XSS
- `~` **历史窗口 12 → 20 条**：允许更长对话上下文
