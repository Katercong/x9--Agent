...
def youtube_local_stats(
    ...
    return {
        ...
        "manual_review": leads_query.filter(YoutubeLead.needs_manual_review == 1, YoutubeLead.has_email == 0).count(),
...
def youtube_collection_actors(
    ...
    review_total = leads_query.filter(YoutubeLead.needs_manual_review == 1, YoutubeLead.has_email == 0).count()
...
def list_youtube_leads(
    ...
    if needs_manual_review is not None:
        ...
        if needs_manual_review:
            query = query.filter(YoutubeLead.has_email == 0)
...
def list_youtube_manual_review(
    ...
    department = current_department_code(request)
    query = _department_query(db.query(YoutubeLead), YoutubeLead, department).filter(
        YoutubeLead.needs_manual_review == 1,
        YoutubeLead.has_email == 0,
    )
    if keyword or source_type:
        query = query.join(YoutubeLeadSource, YoutubeLeadSource.lead_id == YoutubeLead.id)
        if keyword:
