...
function renderAssistantMessages() {
  ...
  box.innerHTML = assistantHistory.map((msg) => `
    <div class="assistant-msg ${msg.role === "user" ? "user" : "assistant"}">${escapeHtml(msg.content)}</div>
  `).join("");
...
function bindBioReasonPopover() {
  ...
  // Passive listener: scroll never blocks the compositor thread.
  // rAF-throttled: at most one hide per frame, regardless of scroll event rate.
  let scrollRaf = 0;
  const onScrollHide = () => {
    if (scrollRaf) return;
  ...
  document.addEventListener("scroll", onScrollHide, { capture: true, passive: true });
