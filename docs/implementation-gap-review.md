...
> 对照：[最终需求规格](final-requirements.md)。本文记录事实状态，不等同于目标需求。本文截至 2026-07-28，覆盖 `main` 已合并的工作台与容器化演示交付，以及当前 `feat/review-queue-sql-optimization` 分支中待 review 的队列 SQL 优化。

## 已实现

| 模块 | 当前能力 |
| --- | --- |
| 数据库与迁移 | Docker Compose PostgreSQL、`.env` 配置、Alembic `c4d5e6f7a8b9` 开发 head；SQLite 自动化测试兼容。Compose 默认编排 PostgreSQL、一次性 `migrate` 和 API。 |
...
| 验证 | Python 全量测试为 `88 passed`，前端 Vitest 为 `8 passed`；已验证 Docker 镜像构建、PostgreSQL 迁移至 `c4d5e6f7a8b9`、API 健康检查、静态资源和重复 demo seed。仅有既有 FastAPI `on_event` 弃用警告。 |
