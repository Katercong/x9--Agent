...
| RBAC Foundation | Agent 本地 `AuthUser`、`Department`、成员关系和追加式授权审计已落库；角色为 `operator`、`reviewer`、`admin`。除 `/health` 外的业务 API 均解析 Principal，并按部门范围执行 `401`、`403` 或跨部门 `404`。当前分支的待 review P1 修复先将历史业务部门码回填为无授权目录项，再规范化目录与历史业务行，禁止管理员认领已使用的部门码且保持已授权历史数据可访问。 |
