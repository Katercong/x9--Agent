...
| 终态审核 | DNC 已支持确认/驳回并留痕；确认后的 DNC 是永久阻断。明确拒绝仍是只读终态记录，尚无人工确认 `dropped` 的状态转换；退信/无效邮箱按需求直接忽略，不进入普通审核。 | 补齐明确拒绝确认 `dropped` 的受 RBAC 保护状态转换与审计。 |
...
- 站内通知、负责人、优先级和到期日。
- 拒绝确认 `dropped` 的受 RBAC 保护写接口；确认后的 DNC 保持永久阻断，退信/无效邮箱直接忽略，不进入普通审核。
