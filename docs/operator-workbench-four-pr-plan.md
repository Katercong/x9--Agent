...
- 工作台从 `/auth/me` 读取当前 Agent Principal，显示本地身份和部门角色，并按角色隐藏或禁用操作；后端仍是唯一授权边界。Docker demo 使用虚构 `demo_reviewer` 身份，绝不接收 X9 Cookie。
...
- 普通项可编辑并批准或关闭；明确拒绝由 reviewer/admin 二次确认后置达人为 `dropped` 并关闭相关待办，operator 只读；待确认 DNC 可由人工确认并转为永久 DNC，或驳回并重新进入普通 Agent 审核；模型失败可由人工明确重试。上述操作均不提供发送或导出。
