...
- 最近全量验证：`python -m pytest -q`，结果为 `70 passed`；只有 FastAPI `on_event` 既有弃用警告。本地 PostgreSQL 已升级到 `ab12cd34ef56 (head)`，关键完整性约束与“每条回复一条最终决定”均已实际验证。
...
- 人工审核后端已提供待审队列、普通回复决议、最终草稿和导出快照审计；每条回复只能形成一个最终决定，审核后不可重跑。`actor_id` 目前只用于留痕，不构成认证或授权。
- DNC 待确认会立即阻断已有普通外联待办，避免人工沿旧任务继续联系。
