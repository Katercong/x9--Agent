...
> 最后更新：2026-07-20。目标规格见 [final-requirements.md](final-requirements.md)，当前实现范围见 [implementation-gap-review.md](implementation-gap-review.md)。本文只描述已验证的代码基线和接手顺序。

## 代码基线

- 远端主分支基线：`main` 的 `29bd7c1 fix: 修复 PostgreSQL 初始迁移数据完整性约束`。
- 当前文档分支：`docs/project-documentation`，用于归档项目开发资料。
- 最近全量验证：`python -m pytest -q`，结果为 `64 passed`；只有 FastAPI `on_event` 既有弃用警告。
...
- 规则分类、DNC、退信、拒绝、资料不足和模型异常均有对应分流。
...
2. 建设人工审核工作台与审核决定/导出审计，保持“系统不自动外发”的边界。
3. 建设用户、部门成员关系与 RBAC，再让审核权限落到实际接口。
