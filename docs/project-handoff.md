...
- 当前功能分支为 `feat/rbac-foundation`：已推送 `de227f5`（授权目录与策略）、`5f0f1f1`（身份 Adapter）、`5307480`（读范围）、`3e34752`（写范围）、`3dcfdbb`（管理员授权审计）、`7b4179f`（身份感知工作台、受控 demo 与文档）、`7f44b0e`（Compose demo 身份默认值）和 `5b343a3`（部门目录与业务归属边界）；当前工作区另有 P1 可访问性修复，等待 review 后提交：规范化历史目录与业务部门码，避免授权目录与严格范围筛选脱节。
...
- `departments` 是受保护的业务部门码目录：回填迁移会为历史业务码创建无成员关系的启用目录项，后续迁移会规范化目录与全部历史业务行的部门码。管理员只能创建从未使用的部门码；创建或迁移达人时，目标目录必须存在、启用且调用者在目标范围具备 `creator:manage`，防止管理员认领其他部门的既有业务数据，并保证已授予范围可读取历史数据。
