...
> 当前进度（2026-07-29）：PR1 至 PR6 与 P1 历史部门码修复已完成并推送；已完成独立 Docker Compose 的迁移、健康检查、工作台、demo 身份、六类队列和重复 seed 幂等性演练。本轮 P2 修复在当前工作区等待 review：并发创建同名全新部门触发唯一约束时，接口回滚失败事务并稳定返回 `409`。本机 demo 的未配置 X9 密钥使用有效空 JSON 默认值，不会影响 `demo` Adapter；真实 X9 签名断言出口仍是外部前置条件。
