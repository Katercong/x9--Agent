...
> 当前进度（2026-07-29）：PR1 至 PR6 已完成并推送；已完成独立 Docker Compose 的迁移、健康检查、工作台、demo 身份、六类队列和重复 seed 幂等性演练。本轮 P1 部门目录修复已在当前分支完成、等待 review：先回填目录，再以前向迁移规范化目录与历史业务行的部门码，阻止认领已有业务部门码，并收紧达人部门写入。本机 demo 的未配置 X9 密钥使用有效空 JSON 默认值，不会影响 `demo` Adapter；真实 X9 签名断言出口仍是外部前置条件。
...
- 新增 Alembic revision：从所有含 `department_code` 的领域表回填规范化部门码；只创建启用目录项，不创建成员关系或授权审计。后续前向 revision 会规范化目录和全部历史业务行，确保严格 SQL 部门范围筛选与目录码一致。两条 revision 的 downgrade 均保留规范化后的目录/业务数据，避免破坏后续授权关系或重新引入越权。
...
- 回归覆盖遗留业务码认领、全新部门创建、活跃目录、未知/停用目录、SQLite upgrade/downgrade 回填与规范化、授权后历史回复详情/队列可访问性、隔离 PostgreSQL 空库升级与全部前后端测试。
