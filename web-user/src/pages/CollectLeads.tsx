...
export default function CollectLeads() {
  ...
  const columns: Column<ObservationItem>[] = [
    ...
    { key: 'followers', header: '粉丝', align: 'right', cell: (r) => <span className="text-xs num text-text">{r.followers_raw || '—'}</span> },
    {
      key: 'email',
      header: '邮箱',
      cell: (r) => (r.lead?.email ? <span className="text-xs num text-text">{maskEmail(r.lead.email)}</span> : <span className="text-xxs text-muted">—</span>),
    },
    {
      key: 'links',
      header: '外链',
    ...
    { key: 'status', header: '状态', cell: (r) => <span className="text-xs text-text">{r.lead?.current_status || '—'}</span> },
  ];
      ...
      >
        <Reveal i={1}>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            <KpiCard label="线索总数" value={num(bucket?.total ?? items.length)} icon={Search} iconBg={A.dim} iconColor={A.key} />
            <KpiCard label="今日新增" value={num(bucket?.today ?? 0)} icon={CalendarDays} iconBg="rgba(6,182,212,0.14)" iconColor="#06b6d4" />
            <KpiCard label="有邮箱" value={num(d.withEmail)} icon={Mail} iconBg="rgba(16,185,129,0.14)" iconColor="#10b981" />
        ...
        <Reveal i={3}>
          <div className="card mt-4">
            <div className="px-4 py-3 border-b border-border flex items-center justify-between">
              <h3 className="text-sm font-semibold text-text">线索明细 · {num(items.length)} 条</h3>
