...
export const superMenu: MenuEntry[] = [
  ...
  { key: 'a-emails', label: '邮件跟踪', to: '/a/emails', icon: Mail },
  { key: 'a-email-auto', label: '自动邮件控制台', to: '/a/email-auto', icon: CalendarClock },
...
export const pageMeta: Record<string, { title: string; subtitle: string }> = {
  ...
  '/a/emails': { title: '达人邮件跟踪（全局）', subtitle: '全部门建联邮件、回复同步与发件邮箱授权状态' },
  '/a/email-auto': { title: '自动邮件控制台', subtitle: '按时间窗口、邮箱额度和达人筛选条件自动生成并发送建联邮件' },
