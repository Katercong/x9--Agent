...
def _clean_row(row: dict[str, Any], row_index: int, metadata: dict[str, Any]) -> dict[str, Any]:
    ...
    review_reasons = _review_reasons(row)
...
def _new_lead(item: dict[str, Any], *, department: str, now: datetime) -> YoutubeLead:
    ...
    return YoutubeLead(
        ...
        emails_json=_json_dumps(item.get("emails") or []),
        has_email=1 if item.get("email") else 0,
        needs_manual_review=1 if item.get("review_reasons") else 0,
        review_reasons_json=_json_dumps(item.get("review_reasons") or []),
...
def _merge_lead(lead: YoutubeLead, item: dict[str, Any], *, now: datetime) -> None:
    ...
    reasons = _merge_unique(_json_load_list(lead.review_reasons_json), item.get("review_reasons") or [])
    ...
    lead.manual_review_url = lead.manual_review_url or item.get("manual_review_url") or None
