...
const I18N = {
  zh: {
    ...
    'leads.title': '最近线索',
    'leads.empty': '暂无邮箱线索。',
    'leads.emptyMeta': '只保存有可见邮箱且粉丝达标的主页。',
  ...
  en: {
    ...
    'filters.email': 'Require visible email',
    ...
    'leads.emptyMeta': 'Only profiles with visible email and enough followers are saved.',
...
function evaluateProfileAgainstFilters(profile, filters) {
  ...
  const hasEmail = Array.isArray(profile.emails) && profile.emails.length > 0;
  if (normalizedFilters.requireEmail && !hasEmail) {
    return {
      qualified: false,
      reason: 'no_email',
    message: '没有可见有效邮箱'
  ...
  return {
    qualified: true,
    reason: 'qualified',
    message: `有可见邮箱且粉丝数 >= ${normalizedFilters.minFollowers}`
  };
}
...
function buildX9ProfileObservation(profile, keyword, source, currentUrl, title, evaluation) {
  ...
  const qualified = Boolean(evaluation?.qualified);
  const currentStatus = qualified ? 'prospect' : 'dropped';
  return {
    ...
    lead_status: qualified ? 'qualified' : 'skipped',
...
let x9UploadSyncTimerId = null;

function startX9UploadSync() {
  if (x9UploadSyncTimerId) {
    return;
  }
  syncX9StoredState('panel_open').catch(() => undefined);
  x9UploadSyncTimerId = window.setInterval(() => {
    syncX9StoredState('interval').catch(() => undefined);
  }, X9_UPLOAD_SYNC_INTERVAL_MS);
}

async function syncX9StoredState(reason) {
  const state = await getState();
  const stored = await chrome.storage.local.get([X9_UPLOADED_OBSERVATION_KEYS]).catch(() => ({}));
  const uploaded = new Set(Array.isArray(stored[X9_UPLOADED_OBSERVATION_KEYS]) ? stored[X9_UPLOADED_OBSERVATION_KEYS] : []);
  const pending = buildX9BackfillObservations(state)
    .map((payload) => ({ payload, key: buildX9ObservationKey(payload) }))
    .filter((item) => item.payload && item.key && !uploaded.has(item.key))
    .slice(0, 100);

  let sent = 0;
  let lastError = '';
  for (const item of pending) {
    const result = await postX9Observation(item.payload);
    if (result.ok) {
  ...
  await chrome.storage.local.set({
    [X9_UPLOADED_OBSERVATION_KEYS]: Array.from(uploaded).slice(-5000),
    x9UploadSyncStatus: {
      ok: !lastError,
      reason,
      sent,
      pending: Math.max(0, pending.length - sent),
      error: lastError,
      synced_at: now()
    }
  }).catch(() => undefined);
...
function buildX9BackfillObservations(state) {
  ...
  for (const lead of state.leads || []) {
    const payload = buildX9ProfileObservation(
      ...
      lead.search_keyword,
      sourceFromStoredProfile(lead),
      lead.profile_url,
      lead.source_video_title || '',
      { qualified: true, reason: 'qualified', message: lead.lead_status || 'lead_saved' }
    );
    if (payload) {
      payload.collected_at = lead.collected_at || lead.last_seen_at || payload.collected_at;
  ...
  for (const skipped of state.skippedProfiles || []) {
    const payload = buildX9ProfileObservation(
      profileFromLead(skipped),
      skipped.search_keyword,
      sourceFromStoredProfile(skipped),
      skipped.profile_url,
      skipped.source_video_title || '',
      { qualified: false, reason: skipped.reason || 'skipped', message: skipped.reason || 'skipped' }
    );
    if (payload) {
      payload.collected_at = skipped.checked_at || skipped.collected_at || payload.collected_at;
      observations.push(payload);
...
function profileFromLead(item) {
  return {
    username: item.username || item.creator_username || extractUsernameFromTikTokUrl(item.profile_url),
    nickname: item.nickname || '',
    profile_url: item.profile_url || item.creator_profile_url || '',
    bio: item.bio || '',
...
function sourceFromStoredProfile(item) {
  return {
    source_video_url: item.source_video_url || item.video_url || '',
    source_video_title: item.source_video_title || item.video_title || '',
    source_video_description: item.source_video_description || item.video_description || ''
  };
}

function parseX9JsonArray(value) {
...
function buildX9ObservationKey(payload) {
  if (!payload?.creator?.handle) {
    return '';
  }
  return [
    payload.lead_status || payload.current_status || '',
    payload.creator.profile_url || '',
    payload.creator.handle || '',
    payload.creator.email || '',
    payload.search_keyword || '',
    payload.source_video?.video_url || ''
  ].join('|').toLowerCase();
}

async function markX9ObservationUploaded(payload) {
  const key = buildX9ObservationKey(payload);
  if (!key) {
    return;
  }
  const stored = await chrome.storage.local.get([X9_UPLOADED_OBSERVATION_KEYS]).catch(() => ({}));
...
// ═══════════════════════════════════════════════════════════════════
// X9 backend config — popup section that lets the user pin the
// dashboard URL. Mirrors what x9_relay.js reads from chrome.storage.local.
// ═══════════════════════════════════════════════════════════════════
...
async function x9DetectBackend(candidates) {
  for (const base of candidates) {
    try {
      const r = await fetch(x9JoinPath(base, "/health"), { method: "GET" });
      if (r.ok) return base;
    } catch (e) {
      // try next
    }
...
async function x9LoadBackendUI() {
  ...
  if (override) {
    ...
    status.textContent = `当前活动:${active}`;
  } else {
    status.textContent = "未连接";
  }
}

async function x9SaveBackend() {
  const input = document.getElementById("x9BackendInput");
  const status = document.getElementById("x9BackendStatus");
  ...
  if (!val) {
    ...
    status.textContent = "已清除,下次心跳会自动检测";
    x9LoadBackendUI();
    return;
  }
  status.textContent = "测试中…";
  // Verify it works before saving — fail loudly if the user pasted a wrong URL.
  try {
    const r = await fetch(x9JoinPath(val, "/health"), { method: "GET" });
  ...
  await chrome.storage.local.set({
    [X9_API_BASE_KEY]: val,
    [X9_API_BASE_ACTIVE_KEY]: val,
  });
  status.textContent = `已保存:${val}`;
}

async function x9DetectBackendUI() {
  const status = document.getElementById("x9BackendStatus");
  if (!status) return;
  ...
  if (found) {
    ...
    status.textContent = `找到:${found}`;
  } else {
    status.textContent = "所有候选都连不上";
  }
}

document.addEventListener("DOMContentLoaded", () => {
  x9LoadBackendUI();
  document.getElementById("x9BackendSave")?.addEventListener("click", x9SaveBackend);
  document.getElementById("x9BackendDetect")?.addEventListener("click", x9DetectBackendUI);
});
